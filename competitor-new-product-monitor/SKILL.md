...
24. **看板图片必须带真实宽高、布局不能用 CSS column-count**：`<img>` 不带尺寸时懒加载前高度为 0，
    图片解码后骤增几百像素；CSS 多列布局在内容高度变化时会重新平衡列高、把卡片跨列搬走，
    滚动时就表现为卡片乱跳、图片「闪烁随机更换」。修法（`gen_dashboard.py` 已实现）：
    ① 生成时读本地图片真实宽高（PNG/JPEG 二进制解析）写进 `<img width height>`；
    ② 布局改为 JS 分列瀑布流（每列独立 flex 容器，卡片按「最矮列优先」分配，永不跨列）。
    另注意：下载的图可能实际是 PNG/WebP 但扩展名是 .jpg（浏览器按内容嗅探能正常显示）
